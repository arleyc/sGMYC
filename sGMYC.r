...
#open tree file Pithecopus
pithe<-read.nexus("hypo.tre")

#Remove outgroup
outgr<-c("9807H_LambariDoeste")
drop.tip(pithe,outgr)->pithe
class(pithe)<-"phylo"
class(pithe)

#open tree file
para<-read.nexus("para.tre")

#Remove outgroup
outgr<-c("Hap_1")
drop.tip(para,outgr)->para
class(para)<-"phylo"
class(para)
...
sGMYC(phy = para, ntrees = 1, ncores = 1, nreps=200, subsamp=2) -> resultados
